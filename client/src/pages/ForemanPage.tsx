...
export default function ForemanPage() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        setIsPending(true);
        setTimeout(() => {
            setMessages(prev => [...prev, { role: "assistant", content: "AI assistant is being migrated to Lovable Cloud. This feature will be available soon!" }]);
            setIsPending(false);
        }, 500);
    };
        ...
        <div className={styles.page}>
            ...
            <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 280px)", background: "white", borderRadius: "12px", border: "1px solid #e2e8f0", overflow: "hidden" }}>
                <div style={{ flex: 1, overflowY: "auto", padding: "24px" }}>
                    {messages.map((msg, idx) => (
                        <div key={idx} style={{ display: "flex", gap: "12px", marginBottom: "20px", justifyContent: msg.role === "user" ? "flex-end" : "flex-start" }}>
                            ...
                            <div style={{ maxWidth: "70%", padding: "12px 16px", borderRadius: msg.role === "user" ? "16px 16px 4px 16px" : "16px 16px 16px 4px", background: msg.role === "user" ? "linear-gradient(135deg, #0d9488 0%, #0f766e 100%)" : "#f1f5f9", color: msg.role === "user" ? "white" : "#1e293b", whiteSpace: "pre-wrap", lineHeight: 1.5 }}>
                                {msg.content}
                            </div>
                            {msg.role === "user" && (
                ...
                <form onSubmit={handleSubmit} style={{ padding: "16px 24px", borderTop: "1px solid #e2e8f0", display: "flex", gap: "12px" }}>
                    <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask Foreman AI anything..." className={styles.input} style={{ flex: 1, padding: "12px 16px" }} disabled={isPending} />
                    <button type="submit" className={styles.createBtn} disabled={isPending || !input.trim()}><Send size={18} /></button>
                </form>
            </div>
        </div>
    );
