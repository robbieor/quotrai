...
function KPICard({ label, value, subMetric, context, contextType = "neutral", onClick }: KPICardProps) {
  return (
    <div
      className={cn(
        "bg-card border border-border rounded-lg p-3 transition-all duration-200 min-w-[170px] shrink-0 snap-start sm:min-w-0 sm:shrink group relative border-l-[3px] border-l-primary/60",
        onClick && "cursor-pointer hover:border-primary/40 hover:bg-muted/20 hover:shadow-sm hover:border-l-primary"
      )}
      onClick={onClick}
    >
...
export function KPIStrip({ data, isLoading, onDrillDown }: KPIStripProps) {
  ...
  if (isLoading) {
    return (
      <div className="flex gap-2.5 overflow-x-auto snap-x snap-mandatory scrollbar-none pb-1 sm:grid sm:gap-3 sm:grid-cols-3 lg:grid-cols-6 sm:overflow-visible sm:pb-0">
        {[...Array(6)].map((_, i) => (
  ...
  return (
    <div className="flex gap-2.5 overflow-x-auto snap-x snap-mandatory scrollbar-none pb-1 sm:grid sm:gap-3 sm:grid-cols-3 lg:grid-cols-6 sm:overflow-visible sm:pb-0">
      ...
      />
      <KPICard
        label={`Money Out ${suffix}`}
        value={formatCurrency(data.moneyOut || 0)}
        subMetric={`${data.moneyOutCount || 0} expense${(data.moneyOutCount || 0) !== 1 ? "s" : ""}`}
        context={data.unreviewedReceipts > 0 ? `${data.unreviewedReceipts} receipt${data.unreviewedReceipts > 1 ? "s" : ""} to review` : "All filed"}
        contextType={data.unreviewedReceipts > 0 ? "negative" : "positive"}
