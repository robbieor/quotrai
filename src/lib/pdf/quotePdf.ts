...
export async function generateQuotePdf(
  ...
  // Tax: per-line breakdown when mixed, single line otherwise
  const hasPerLine = quote.quote_items.some((i: any) => i.tax_rate !== undefined && i.tax_rate !== null);
  let cursorY = finalY + 7;
  if (hasPerLine) {
    const { breakdown, uniformRate, taxAmount } = calculateTotals(
      quote.quote_items.map((i: any) => ({
    ...
    doc.setTextColor(100, 100, 100);
    doc.text(`Tax (${Number(quote.tax_rate)}%):`, pageWidth - 70, cursorY);
    doc.setTextColor(0, 0, 0);
    doc.text(`${currencySymbol}${Number(quote.tax_amount).toFixed(2)}`, pageWidth - 20, cursorY, { align: "right" });
  }
  ...
  doc.text(`${currencySymbol}${Number(quote.total).toFixed(2)}`, pageWidth - 20, cursorY + 13, { align: "right" });
