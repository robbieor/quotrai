  ...
  try {
    ...
    for (const [team_id, count] of fByTeamQ) {
      ...
      suggestions.push({
        ...
        description:
          `You've manually followed up on ${count} quotes in the last 30 days. Foreman can send these for you 3 days after the quote goes out, with a friendly tone, unless the customer's already replied.`,
    ...
    for (const [team_id, count] of fByTeamI) {
      ...
      suggestions.push({
        ...
        description:
          `You've manually chased ${count} overdue invoices recently. Foreman can send a polite reminder the day after an invoice goes overdue, then escalate at 7 and 14 days.`,
    ...
    for (const [team_id, count] of recurringByTeam) {
      suggestions.push({
        ...
        description:
          `${count} of your customers have booked 3+ jobs in 90 days. Foreman can flag them as recurring, suggest service plans, and remind you to schedule the next visit before they call.`,
    ...
    for (const [team_id, accepted] of accByTeam) {
      ...
      if (rate >= 0.8) {
        suggestions.push({
          ...
          description: `${Math.round(rate * 100)}% of your accepted quotes get turned into jobs. Foreman can do that step for you the moment a quote is accepted, and you'll just see the new job ready to schedule.`,
