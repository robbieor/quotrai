    ...
    for (const profile of profiles) {
      ...
      try {
        ...
        const { error: enqueueError } = await supabase.rpc("enqueue_email", {
          ...
          payload: { message_id: messageId, to: profile.email, from: `Quotr <noreply@${FROM_DOMAIN}>`, sender_domain: SENDER_DOMAIN, subject: `${firstName}, we miss you on Quotr! 🛠️`, html, text: `Hey ${firstName}, we noticed you haven't been on Quotr recently. Jump back in!`, purpose: "transactional", label: "churn-reengagement", queued_at: new Date().toISOString() },
        });
