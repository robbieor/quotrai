  ...
  try {
    ...
    for (const profile of profiles) {
      ...
      try {
        ...
        const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/></head><body style="margin:0;padding:0;background:#f4f4f5;font-family:'Manrope',Arial,sans-serif"><table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:32px 16px"><table width="100%" style="max-width:560px;background:#fff;border-radius:12px;overflow:hidden"><tr><td style="background:#0f172a;padding:30px 32px;text-align:center"><img src="https://revamo.ai/foreman-logo.png" alt="Revamo" width="140" style="display:block;margin:0 auto;" /></td></tr><tr><td style="padding:32px"><h1 style="margin:0 0 16px;font-size:20px;color:#0f172a">Hey ${firstName},</h1><p style="margin:0 0 12px;color:#64748b;font-size:14px;line-height:1.6">We noticed you haven't been on Revamo recently. Your business doesn't stop — and neither should your quoting and invoicing.</p><div style="background:#f0fdf4;border-left:4px solid #00E6A0;padding:16px;margin:20px 0;border-radius:4px;"><p style="margin:0;font-weight:bold;color:#0f172a;">Quick wins waiting for you:</p><ul style="margin:8px 0 0;padding-left:20px;color:#64748b;font-size:14px;line-height:1.6;"><li>Create quotes in seconds with templates</li><li>Send invoices and get paid faster</li><li>Track your jobs and never miss an appointment</li></ul></div><div style="text-align:center;margin:24px 0;"><a href="https://revamo.ai/dashboard" style="display:inline-block;background:#00E6A0;color:#0f172a;padding:12px 28px;border-radius:12px;text-decoration:none;font-weight:600;font-size:14px;">Jump Back In →</a></div><p style="color:#64748b;font-size:14px;margin-top:24px;">Need help? Chat with Revamo AI, your AI assistant, anytime.</p></td></tr><tr><td style="padding:16px 32px;text-align:center;font-size:12px;color:#94a3b8;border-top:1px solid #e2e8f0">© ${new Date().getFullYear()} Revamo · <a href="mailto:support@foreman.ie" style="color:#94a3b8">support@foreman.ie</a></td></tr></table></td></tr></table></body></html>`;
        ...
        const { error: enqueueError } = await supabase.rpc("enqueue_email", {
          ...
          payload: { message_id: messageId, to: profile.email, from: `Revamo <support@${FROM_DOMAIN}>`, sender_domain: SENDER_DOMAIN, subject: `${firstName}, we miss you on Revamo! 🛠️`, html, text: `Hey ${firstName}, we noticed you haven't been on Revamo recently. Jump back in!`, purpose: "transactional", label: "churn-reengagement", queued_at: new Date().toISOString() },
        });
