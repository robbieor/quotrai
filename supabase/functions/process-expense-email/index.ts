  ...
  try {
    ...
    if (outboundEnabled) {
      try {
        ...
        const confirmHtml = `<div style="font-family:'Manrope',sans-serif;max-width:500px;margin:0 auto;padding:20px;"><h2 style="color:#00E6A0;margin-bottom:16px;">Expense Logged ✅</h2><p>Your forwarded invoice has been automatically added to your expenses:</p><table style="width:100%;border-collapse:collapse;margin:16px 0;"><tr><td style="padding:8px 0;color:#64748b;">Description</td><td style="padding:8px 0;font-weight:bold;">${description}</td></tr><tr><td style="padding:8px 0;color:#64748b;">Amount</td><td style="padding:8px 0;font-weight:bold;">${amount.toFixed(2)}</td></tr><tr><td style="padding:8px 0;color:#64748b;">Vendor</td><td style="padding:8px 0;">${vendor || "—"}</td></tr><tr><td style="padding:8px 0;color:#64748b;">Date</td><td style="padding:8px 0;">${expenseDate}</td></tr><tr><td style="padding:8px 0;color:#64748b;">Category</td><td style="padding:8px 0;">${category}</td></tr></table><p style="color:#64748b;font-size:14px;">If this doesn't look right, just edit it in your Foreman dashboard.</p></div>`;
        ...
        await adminSupabase.rpc("enqueue_email", {
          ...
          payload: { message_id: messageId, to: senderAddr, from: `Foreman Expenses <noreply@${FROM_DOMAIN}>`, sender_domain: SENDER_DOMAIN, subject: `✅ Expense logged: ${description}`, html: confirmHtml, text: `Expense logged: ${description} - ${amount.toFixed(2)}`, purpose: "transactional", label: "expense-confirmation", queued_at: new Date().toISOString() },
        });
