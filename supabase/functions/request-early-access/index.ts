...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    await supabase.rpc("enqueue_email", {
      queue_name: "transactional_emails",
      payload: { message_id: confirmMsgId, to: email, from: `Quotr <hello@${FROM_DOMAIN}>`, sender_domain: SENDER_DOMAIN, subject: "Welcome to the Quotr Beta Waitlist! 🚀", html: confirmHtml, text: `Hi ${contactName}, thank you for your interest in Quotr! We'll be in touch within 24-48 hours.`, purpose: "transactional", label: "early-access-confirm", queued_at: new Date().toISOString() },
    });
