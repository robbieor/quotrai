...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    await supabase.rpc("enqueue_email", {
      ...
      payload: { message_id: adminMsgId, to: "accounts@foreman.ie", from: `Foreman <noreply@${FROM_DOMAIN}>`, sender_domain: SENDER_DOMAIN, subject: `New Early Access Request: ${safe.companyName}`, html: adminHtml, text: `New early access request from ${safe.contactName} at ${safe.companyName}`, purpose: "transactional", label: "early-access-admin", queued_at: new Date().toISOString() },
    });
