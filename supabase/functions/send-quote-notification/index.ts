...
const brandStyles = `
  ...
  .invoice-box { 
    background: linear-gradient(135deg, rgba(0, 255, 178, 0.1), rgba(0, 212, 255, 0.1)); 
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Create Supabase client with service role to fetch quote details
    const supabaseUrl = Deno.env.get("SUPABASE_URL")!;
    const supabaseServiceKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
    const supabase = createClient(supabaseUrl, supabaseServiceKey);

    // Fetch quote with customer and team details
    ...
    // Get team owner's email to send notification
    ...
    if (memberError || !teamMemberships?.length) {
      console.error("Error fetching team owner:", memberError);
      return new Response(
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p>${isAccepted 
                ? "Great news! Your quote has been accepted by the customer." 
                : "Your quote has received a response from the customer."}
              </p>
              
              <div style="text-align: center;">
                <span class="status-badge ${isAccepted ? 'status-accepted' : 'status-declined'}">${statusLabel.toUpperCase()}</span>
              </div>
              ...
              <div class="details">
                ...
                <div class="details-row">
                  ...
                  <span>${quote.customer?.name || "Unknown"}</span>
                </div>
                <div class="details-row">
              ...
              ${isAccepted && invoiceNumber ? `
              <div class="invoice-box">
                <strong>✅ Invoice Created:</strong> ${invoiceNumber}<br>
                <small>An invoice has been automatically generated from this quote.</small>
              </div>
              ` : ''}
              
              ${!isAccepted && declineReason ? `
              <div class="reason-box">
                <strong>💬 Customer's Reason:</strong><br>
                ${declineReason}
              </div>
              ` : ''}
              
              <p style="margin-top: 20px;">Log in to your Quotr account to view the details and take any necessary action.</p>
            </div>
            <div class="footer">
