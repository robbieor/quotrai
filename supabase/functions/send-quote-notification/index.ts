...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { error: enqueueError } = await supabase.rpc("enqueue_email", {
      ...
      payload: { message_id: messageId, to: ownerProfile.email, from: `Quotr <noreply@${FROM_DOMAIN}>`, sender_domain: SENDER_DOMAIN, subject: `${emoji} Quote ${quote.display_number} has been ${statusLabel.toLowerCase()}`, html, text: `Quote ${quote.display_number} has been ${statusLabel.toLowerCase()}`, purpose: "transactional", label: "quote-notification", queued_at: new Date().toISOString() },
    });
