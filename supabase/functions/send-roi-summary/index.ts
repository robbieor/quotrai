...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { error: enqueueError } = await supabase.rpc("enqueue_email", {
      ...
      payload: { message_id: messageId, to: data.email, from: `revamo <support@${FROM_DOMAIN}>`, sender_domain: SENDER_DOMAIN, subject: `${firstName}, you could save ${formatCurrency(data.monthlyNetSavings)}/month with revamo`, html: emailHtml, text: `Hey ${firstName}, you could save ${formatCurrency(data.monthlyNetSavings)}/month with revamo.`, purpose: "transactional", label: "roi-summary", queued_at: new Date().toISOString() },
    });
